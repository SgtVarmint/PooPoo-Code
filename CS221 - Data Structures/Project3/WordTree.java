...
public class WordTree {
	...
	private Node addWord(Node root ,String word ,int score){
		if(root==null){
			Node node = new Node();//new node to hold the word score and count
			node.word = word;
			node.score = score;
			node.count = 1;
			node.black = false;
			...
			int value = root.word.compareTo(word);//the integer that specifies whether or not the word comes first alphabetically
			if(value<0)
	...
	private Node rotateLeft(Node root) {
		Node temp = root.right;//used for rotating nodes
	...
	/**
	 * Right rotate method for red black tree
	 * @param root
	 * @return
	 */
	private Node rotateRight(Node root) {
		Node temp = root.left;//used for rotating nodes
	...
	/**
	 * flips red to black and of children nodes
	 * @param root
	 */
	...
	private void printAll(PrintWriter out,Node root){
		if(root==null)
			...
			out.println(root.word+"\t" + root.score + "\t" + root.count);
